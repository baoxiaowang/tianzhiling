...
const CHAT_MESSAGE_CACHE_KEY_PREFIX = 'tzl_chat_messages_v1'
const CHAT_MESSAGE_CACHE_INDEX_KEY = 'tzl_chat_messages_index_v1'
...
interface ConversationChatBootstrapResponse extends ConversationMessageListResponse {
  agent?: unknown
  chatQuota?: unknown
}
...
export interface ConversationChatBootstrapResult extends ConversationMessageListResult {
...
export function parseConversationSummary(value: unknown): ConversationSummary {
  ...
  return {
    ...
    agentAccessRole: asString(raw.agentAccessRole) === 'shared' ? 'shared' : 'owner',
...
function asStringArray(value: unknown) {
  ...
  return value.map((item) => stripAssistantMarkup(asString(item)).trim()).filter(Boolean)
}
...
function parseVoicePayload(value: unknown) {
  ...
  const normalizedDurationMs = durationMs > 0 && durationMs <= 10 * 60 * 1000 ? durationMs : 0
...
export async function getConversations(options: { force?: boolean; timeout?: number } = {}) {
  if (options.force) {
  ...
  const promise = fetchConversations(ownerId, requestVersion, options.timeout).finally(() => {
...
export async function getConversationPage(
  ...
  const entryItem = data.entryItem ? parseConversationSummary(data.entryItem) : undefined
...
export async function getEntryConversation(
  ...
  const data = options.timeout
    ? await getWithOptions<EntryConversationResponse>('/api/conversation/entry', {
        timeout: options.timeout,
      })
...
export function updateCachedConversationDefault(agentId: string, isDefault: boolean) {
  const ownerId = getConversationCacheOwnerId()
...
async function fetchConversations(ownerId: string, requestVersion: number, timeout?: number) {
  const data = timeout
...
export async function getConversationMessages(conversationId: string) {
  ...
  return Array.isArray(data.items) ? data.items.map((item) => parseConversationMessage(item)) : []
}
...
export function getCachedConversationMessages(
  ...
  try {
    const raw = Taro.getStorageSync<string>(buildChatMessageCacheKey(ownerId, conversationId))
    const stored = raw
      ? (JSON.parse(raw) as {
          ownerId?: string
          expiresAt?: number
...
function saveCachedConversationMessages(
  ...
  try {
    ...
    const existingIndex = readChatMessageCacheIndex().filter((key) => key !== cacheKey)
    const nextIndex = [...existingIndex, cacheKey]
    const expiredKeys = nextIndex.slice(0, Math.max(0, nextIndex.length - CHAT_MESSAGE_CACHE_LIMIT))
...
function invalidateCachedConversationMessages(conversationId: string) {
  ...
  try {
    ...
    Taro.setStorageSync(
      CHAT_MESSAGE_CACHE_INDEX_KEY,
      JSON.stringify(readChatMessageCacheIndex().filter((key) => key !== cacheKey))
    )
  } catch {
...
function parseConversationBootstrapAgent(value: unknown): ConversationBootstrapAgent | undefined {
  const raw = asRecord(value)
...
export async function markConversationMessageMemory(conversationId: string, messageId: string) {
  await post(
    `/api/conversation/${encodeURIComponent(conversationId)}/messages/${encodeURIComponent(
      messageId
    )}/memory`,
...
export async function generateConversationMessageVoice(conversationId: string, messageId: string) {
