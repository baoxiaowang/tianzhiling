  ...
  <div class="user-value-page">
    ...
    <a-spin :loading="loading">
      <div class="user-value-page__definition">
        ...
        <strong
          >注册用户产值 = 该注册月用户累计实付金额 ÷ 该月新增注册用户数</strong
        >
      ...
      <section class="user-value-page__summary">
        ...
        <article>
          ...
          <small
            >{{ formatNumber(selectedCohort?.payingUsers) }} 位付费用户</small
          >
        </article>
      </section>
      ...
      >
        ...
        >
          <template #columns>
            <a-table-column title="注册月份" data-index="month" :width="150">
            ...
            <a-table-column title="7 日产值" :width="120">
              <template #cell="{ record }">
                {{
                  record.is7DayMature ? formatMoney(record.value7Day) : '观察中'
                }}
              </template>
            </a-table-column>
...
<script lang="ts" setup>
  ...
  import type { AdminUserValueReportDTO } from '@tzl/shared';
  import { queryUserValueReport } from '@/api/operations';
  ...
  const cohortChartOption = computed(() => {
    ...
    return {
      ...
      yAxis: { type: 'value', name: cohortMetricMeta.value.unit, min: 0 },
      series: [
        {
