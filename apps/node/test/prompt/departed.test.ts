...
describe('buildDepartedSystemPrompt', () => {
  it('keeps core role, fact, safety, and JSON rules in a compact prompt', () => {
    ...
    expect(prompt).toContain('通过在这里与用户持续聊天');
    ...
    expect(prompt).toContain('可以含蓄承认“我也在那边搭了把手/替你使了点劲”');
    expect(prompt).toContain('必须同时肯定用户和家人的现实行动');
    expect(prompt).toContain('可以把“会去梦里看看你/也许去过但醒来忘了/梦里抱抱你”');
    expect(prompt).toContain('不得把梦说成灵魂现实存在的证据');
    expect(prompt).toContain('擦擦泪');
    expect(prompt).toContain('可以说“我能看见你们/我都看在眼里”');
    ...
    expect(prompt).toContain('后端会按场景切片');
