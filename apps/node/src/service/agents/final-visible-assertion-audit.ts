...
const COMPLETED_REAL_WORLD_VISIT =
  /(?:(?:前几天|这几天|昨天|昨晚|今天|刚才|刚刚).{0,12})?(?:我|爸|爸爸|妈|妈妈|爷爷|奶奶|姥姥|姥爷|外公|外婆|老公|老婆)?(?:回来|回去|回家|回到|过去|路过|到了|来到).{0,16}(?:你家|你楼下|楼下|家门口|门口|床边|房间|屋里|家里).{0,30}(?:看见|看到|站了|待了|坐了|陪了|看了)|(?:(?:我|爸|爸爸|妈|妈妈|爷爷|奶奶|姥姥|姥爷|外公|外婆|老公|老婆).{0,10})?(?:在|到过|来过).{0,8}(?:你楼下|楼下|你家|家门口|床边|房间|屋里).{0,20}(?:看你|等你|陪你|站着|待着)/;
const NON_ASSERTIVE_VISIT_PREFIX =
  /(?:没(?:有)?|未曾|不能|没法|无法|不可能|想|真想|多想|希望|盼着|但愿|恨不得|要是|如果|假如|梦里|梦中|做梦|以后|将来|下次|哪天|有机会|打算|准备|会).{0,10}$/u;
...
export function auditVisibleReplyAssertions(options: {
  ...
  const completedRealWorldVisit = findCompletedRealWorldVisit(content);
  if (
    !PHYSICAL_BOUNDARY.test(content) &&
    (PHYSICAL_ASSERTION.test(content) || completedRealWorldVisit)
  ) {
    add({
      ...
      repairGoal:
        '保留想靠近、条件或梦里的表达，不把现实到访、触碰或化身写成已经发生的事实',
    });
  }
