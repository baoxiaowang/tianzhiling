...
const DREAM_VISIT_ASSERTION = new RegExp(
  `(?:${ROLE}.{0,12}(?:今晚|今夜|明晚|昨晚|刚才|等会儿|待会儿)?.{0,8}(?:一定|肯定|会|就|已经|真的)?(?:来|去|到|进|入|托).{0,8}(?:你)?(?:梦里|梦中|梦)|(?:今晚|今夜|明晚|昨晚|刚才).{0,10}${ROLE}.{0,8}(?:来|去|到|进|入|托).{0,8}(?:梦里|梦中|梦)|(?:那个梦|你梦里|梦中的).{0,10}(?:是我|就是我|我去的))`
);
const DREAM_VISIT_WITH_LOCK =
  /(?:今晚|今夜|明晚|等会儿|待会儿).{0,12}(?:我|爸|爸爸|妈|妈妈|爷爷|奶奶|姥姥|姥爷|外公|外婆|老公|老婆)?(?:就|会|一定|肯定)?(?:来|去|到|进|看看|陪陪)/;
...
export function auditVisibleReplyAssertions(options: {
  ...
  if (!DEATH_BOUNDARY.test(content) && DEATH_EXPERIENCE.test(content)) {
    add({
      code: 'unsupported_death_experience',
      problem: '正文独立出现了需要证据的临终体验、心理或原因断言',
      evidence: matchEvidence(content, DEATH_EXPERIENCE),
