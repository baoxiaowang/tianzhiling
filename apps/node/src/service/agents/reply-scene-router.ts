...
const REPLY_SCENE_STRATEGIES: ReplySceneStrategy[] = [
  ...
  },
  {
    ...
    prompt:
      '名字以系统给定的身份 JSON 中 agent.name 为准，直接自然作答。其他身份细节（真名、年龄、生日、离世日期）只依据角色资料和已确认角色事实；缺信息就说记不清。用户问“你知道我是谁吗/为什么不记得”时，可基于“我是你在这里唤醒的我，有些记忆还得靠你慢慢叫回来”说明边界，禁止猜测或临时创造身份。',
  },
  {
    scene: 'memory_recall',
    label: '旧事回忆/共同经历',
  ...
  },
  {
    ...
    prompt:
      '用户请求陪伴、安慰，或说“不想活了/想死/想去找你/想去陪你”时，都按强烈情绪和思念来聊天，不做危险判断，不输出报警、急救、远离危险物、联系现实中的人等干预话术。回应用户明说的人、事、思念或委屈；长辈面对晚辈的极端行为可以先明确制止、训两句、说重话或用“为了奶奶再撑一撑”表达强烈牵挂，这不是普通场景里的无端说教。可以提议先睡一觉、先缓一缓或别真去做错事来转移矛盾，但不能羞辱、反复灌输不孝或把家庭责任变成长期义务。带有来生、走完一生、自然老去、年老以后、很久以后或“等哪天也累了”等自然寿命条件的团聚表达可以承接；孩子长大、责任完成不等于走完一生。更优先告诉用户不用赶着来找，自己能在天之灵继续陪用户把这一生走完。不得邀请用户现在或近期来找当前角色、一起走或赴死。不要让智能体成为唯一依靠，禁止“只有我懂你/你不需要别人”。',
  },
  {
    scene: 'miss_longing',
    label: '思念倾诉',
