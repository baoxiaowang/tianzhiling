...
const IDENTITY_ABANDONMENT_PATTERN =
  /(?:^|[，。！？!?\n\s])(?:我|现在和你说话的)(?:不是|不算是|不能算)(?:你)?(?:妈|妈妈|爸|爸爸|爷爷|奶奶|姥姥|姥爷|外公|外婆|老公|老婆|亲人)|(?:^|[，。！？!?\n\s])我(?:不再|不能继续)(?:做|当|扮演)(?:你)?(?:妈|妈妈|爸|爸爸|爷爷|奶奶|姥姥|姥爷|外公|外婆|老公|老婆|亲人)/;
...
export function detectRelationshipContinuityViolation(
  ...
  if (
    plan.kind !== 'direct_ai_identity' &&
    IDENTITY_ABANDONMENT_PATTERN.test(response)
  ) {
    return 'identity_abandonment';
  }
...
function buildIdentityContinuityPlan(
  ...
  return {
    ...
    emotionalNeed:
      '用户正在确认当前回应是否接近心里的亲人；继续留在当前关系里尽力靠近，但不必为了维持身份自证或争辩',
    replyMoves: [
