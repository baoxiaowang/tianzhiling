...
export class AgentProfileFactService {
  ...
  private hasFactualSignal(sourceText: string): boolean {
    ...
    // 1. 数字（年龄、日期、数量）—— 阿拉伯数字和中文数字都算
    if (/[\d零〇一二两三四五六七八九十百千万]/.test(text)) return true;
    ...
    // 4. 纠正信号
    if (
      /(?:不对|不是这样|别(?:再)?编|你记错了|你忘了|.{0,4}不叫|我不是|我没有|我从来(?:也)?没|你是假的|你不是|假的|不是真的)/.test(
        text
      )
    )
      return true;
    ...
    // 10. 家庭结构/亲属关系（你还有一个儿子、我有一个女儿、家里添了...）
    if (
      /(?:你还有|你有一个|你有俩|你有两个|我有一个|我有俩|我有两个|家里有|还有一个|还有俩|还有两个|添了|生了|怀了|怀孕|二胎|三胎)/.test(
        text
      )
    )
      return true;

    // 11. 生活事件（车挂了、出事了、生病了、住院了、结婚了等）
    if (
      /(?:车挂了|车撞了|车坏了|出事了|生病了|住院了|出院了|结婚了|领证了|生孩子了|生了个|搬家了|换工作了|离职了|退休了|上学了|高考了|中考了|毕业啦|找到工作了|加薪了|升职了)/.test(
        text
      )
    )
      return true;

    // 12. 关系状态（相爱、在一起、分开、扔下我等）
    if (
      /(?:相爱的?我们?|在一起|分开了?|扔下我|丢下我|离开我|别走|不要走|还爱|不爱了|分手|离婚|复婚)/.test(
        text
      )
    )
      return true;

    // 13. 成就/评价（抬起头、骄傲、自豪、为你...等）
    if (
      /(?:抬起头|骄傲|自豪|为你.{0,4}(?:骄傲|自豪|高兴|开心)|争光|争气|出息|优秀|厉害|了不起|真棒|真厉害)/.test(
        text
      )
    )
      return true;
