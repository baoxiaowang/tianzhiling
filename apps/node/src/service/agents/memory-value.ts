...
export interface MemoryValueDecision {
  ...
  date?: {
    /**
     * birthday_observance：只知道"每年按哪天过生日"、不知道出生年份的年度生日。
     * 与 birth 分开，避免下游把年度纪念日当成完整出生日期（不能伪造年份）。
     */
    event: 'birth' | 'death' | 'expected_birth' | 'birthday_observance';
    year?: number;
    month?: number;
    day?: number;
    expression?: string;
...
// 说"你已经走了 13 年"也拿不到依据，只能现编日期（实测编出过 2013-09-02）。
// 但 status.* 前缀太宽（同一个前缀下还有"当前心情"这类确实只该作背景的），
// 所以按"前缀 + 离世语义"放行，不整段开放。模型写过 status.deceased、
// status.deceased_since、status.died、status.since_passing 等多种写法。
const DEATH_STATUS_KEY_PATTERN =
  /^(?:status|life_status)\.(?:deceased|died|die|dead|death|decease|passed_away|passing|since_passing|deceased_since|deceased_at|death_date|death_time)/;
...
export function isContextOnlyNamespace(key: string): boolean {
  ...
  if (DEATH_STATUS_KEY_PATTERN.test(normalized)) return false;
...
export const MEMORY_VALUE_PROMPT = [
  ...
  '记忆类型（只记这六类，其余一律不记）：①用户本人的情况——姓名、年龄、婚育、工作、居住、健康、喜好；②本次对话所服务的这位亲人的经历与共同往事——主体用conversationAgentRef，不是用户；③其他家人的关系与出现记录——用用户当时的称呼；④亲人离世与离世时间；⑤话题与事件——这次聊了什么、发生了什么，有时间的写时间；⑥用户提到的承诺与心愿——只记"他提到过"，不写成已经发生。',
  '情绪不记：想你、难过、崩溃、委屈、孤独、心慌等感受，以及"每到清明我就想去看看你""听到医院两个字会发抖"这类带时间或场景的哀伤表达，一律不写。当下状态不记：现在在休息、刚吃完饭、今天有点累、此刻在忙。这两类连"短期背景"都不要建条。',
  '话题与事件要记：用户主动谈起的主题（"上次说到想给妈妈立块碑，还没定"）和确实发生过的事（"中元节去给爸妈烧了纸""妹妹今年考上大学""小孙子九月入园"）。有时间的写时间，没有时间的只记"聊过这件事"，方便下次接上话、不重复问。',
  '家人的信息用用户当时的说法：说"小外孙"就写"小外孙"，说"孙子"就写"孙子"，不替他统一；同一位家人换称呼是两次出现记录，不要当成两个新人，也不要把不同名字的两个人并成一个人。',
  ...
  '区分可信程度、保存价值、时间性质和修改保护。健康情况属于用户本人的信息项，按稳定事实记；一次发作、当下不适这类瞬时状态不记。历史事件不因久远自动失效。',
  ...
  'value必须是完整中文字符串，包括数字事实也必须写成“离世时18岁”这样的事实句，年龄用kind=person,type=age，不从年龄猜测离世日期。离世多久、生日、日期线索用kind=temporal,type=memory，并提供date，不能归为情绪类，也不存在type=death/birth/temporal。',
  '不要从“怎么忍心扔下我”等当下抒情推导被抛弃感、核心哀伤叙事、心理状态或性格；不保存这种心理解释。具体事件也不能附会坚韧、家庭协作等用户未说出的特征。',
  '结构示例（替换为实际人物和证据，勿照存示例）：{"newPeople":[],"decisions":[{"subjectRef":"user:输入中的ID","participants":[],"kind":"person","type":"memory","key":"health.chronic_back_pain","value":"用户有腰疼的老毛病","retention":"durable","certainty":"explicit","timeKind":"stable","operation":"add","reason":"用户本人长期健康情况","evidence":[{"messageId":"当前消息ID","quote":"妈妈腰疼又犯了"}],"protected":false,"salience":2}]}',
