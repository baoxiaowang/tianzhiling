...
export function resolveFinalReplyOutputConstraints(options: {
  ...
  const candidateSegmentCount = options.candidateSegmentCount;

  return {
    ...options.contract.outputConstraints,
    requiredSegmentCount: options.contract.delivery.preferTwoSegments
      ? 2
      : candidateSegmentCount === 1 || candidateSegmentCount === 2
...
function buildReplyTurnContractPrompt(
  ...
  return [
    ...
    contract.boundary.realityDependencyKinds.length
      ...
      : '',
    '本契约是生成与终检的共同输入。必须动作不可被“弱提示”覆盖；长度和双泡不得挤掉必答内容。',
  ]
    .filter(Boolean)
    .join('\n');
}
