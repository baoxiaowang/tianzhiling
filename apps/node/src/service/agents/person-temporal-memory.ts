...
function parseRelativeDuration(
  ...
  const matches: RegExpMatchArray[] = collectRegexMatches(
    text,
    /(?:离开|走了|去世|离世|过世|不在了)[^，。！？!?]{0,12}?([0-9零〇一二两三四五六七八九十百千]+)\s*(?:个)?(年头|年|个?月|周|星期|天)(?:前|了|啦|左右|多了|多|来)?/g
  ).concat(
    collectRegexMatches(
      text,
      /([0-9零〇一二两三四五六七八九十百千]+)\s*(?:个)?(年头|年|个?月|周|星期|天)(?:前|了|啦|左右|多了|多|来)?[^，。！？!?]{0,12}?(?:离开|走|去世|离世|过世|不在)/g
    )
  );
  if (implicitCurrentAgent && !matches.length) {
    const implicitMatch = text.match(
      /(?:已经|都|有|差不多|大约|大概|将近|快)?\s*([0-9零〇一二两三四五六七八九十百千]+)\s*(?:个)?(年头|年|个?月|周|星期|天)(?:前|了|啦|左右|多了|多|来)?(?:[，。！？!?]|$)/
    );
