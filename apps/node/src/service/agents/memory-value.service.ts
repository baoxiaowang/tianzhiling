...
export class MemoryValueService {
  ...
  async propose(
    ...
    const result = await this.openAIService.generateText({
      ...
      // 稳定大块（MEMORY_VALUE_PROMPT）放在最前，动态的任务说明/批量计数放后面：
      // 服务端按前缀缓存，动态内容一旦插在稳定块之前，稳定块就无法成为共享前缀。
      // 这里只调整顺序，三段文本逐字保留，段间换行数不变（latent：生产
      // memoryValue=off，此改动当前无生产收益，仅为将来开启时缓存命中）。
      systemPrompt:
        MEMORY_VALUE_PROMPT +
        '\n' +
