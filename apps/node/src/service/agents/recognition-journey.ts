...
export function buildSubsequentRelativeGreetingPrompt(
  ...
  // 第 3–5 轮窗口内按“指定任务”强制表达：先回应本轮内容，再必须带出这层意思；
  // 只有用户明确拒绝本卡话题才可免。可选措辞保留给窗口外/兼容调用。
  if (options.mandatory) {
    return [
      '# 本轮指定任务（系统内部，不要向用户说明这是任务）',
      `本轮必须先自然回应用户当前说的话，然后带出“刚刚${who}跟我说到你，你能来找我真是太高兴了”这层意思。措辞可以调整，但不要省略、不要只写一半，也不要用反问或承诺代替表达。`,
      '不追问用户来完成这句话，不展开编造对方说过的具体内容，不声称两人之间已经真实互发过消息，也不要把这句话写成关于用户的事实。',
      '只有用户本轮明确拒绝提这位亲人或这类话题时才可以不执行；其他情况（包括用户在讲别的烦心事）也要在回应之后自然带出，不要因为话题沉重就省略。',
    ].join('\n');
  }
  return [
