...
export class TencentVrsVoiceService {
  ...
  private get defaultVoiceGender(): string {
    return (this.config?.defaultVoiceGender || '2').trim() || '2';
  }
  ...
  ensureConfigured(): void {
    if (!this.isEnabled()) {
      throw new AppError(
        ...
        '腾讯云声音复刻未启用，请检查服务端配置 NODE_TENCENT_VRS_ENABLED',
        503
      );
    }
    if (!this.secretId || !this.secretKey) {
      throw new AppError(
        ...
        '腾讯云声音复刻凭证未配置，请检查服务端 NODE_TENCENT_VRS_SECRET_ID / SECRET_KEY',
        503
      );
    }
  }
  ...
  async createVrsTask(input: {
    ...
    const payload: Record<string, unknown> = {
      ...
      VoiceName: voiceName,
      VoiceGender: Number(input.voiceGender ?? this.defaultVoiceGender), // int64
      VoiceLanguage: 1,
      AudioIdList: [audioId],
   ...
   * 取消任务（CancelVRSTask 仅能取消进行中的任务，且尚未在文档中完整开放）。
    ...
    if (apiError) {
      ...
      if (/VRSQuotaExhausted/i.test(code)) {
        throw new AppError(
          ...
          '腾讯云声音复刻额度已用尽，请前往腾讯云控制台购买或领取免费额度',
          402,
