  ...
  static Future<XFile?> edit(BuildContext context, XFile source) {
    return Navigator.of(context).push<XFile>(
      MaterialPageRoute<XFile>(
        builder: (context) => AvatarEditorPage(source: source),
      ),
    );
  }
...
class _AvatarEditorPageState extends State<AvatarEditorPage> {
  ...
  Future<void> _completeEdit() async {
    ...
    try {
      ...
      final editedBytes = await compute(_applyAvatarEdit, <String, Object>{
        'bytes': bytes,
      ...
      final file = await _writeEditedAvatarFile(editedBytes);
  ...
  Future<File> _writeEditedAvatarFile(Uint8List bytes) async {
    final file = File(
...
Uint8List _applyAvatarEdit(Map<String, Object> payload) {
  ...
  if (decoded == null) {
    throw StateError('failed to decode avatar image');
  }
