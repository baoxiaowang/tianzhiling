...
- Do not invoke Royal TSX's paid AppleScript interface or use accessibility/UI control, terminal injection, or other automation to select a connection or enter commands.
...
- For a protected release, keep the Royal action to a short approval for the exact immutable commit, confirm a clear success marker, and then continue the deployment through restricted SSH. If a required manual action cannot fit safely in a short command, stop and create a reusable server-side helper before asking the user to continue.
- If direct SSH is not authorized, prepare the shortest possible single-line diagnostic or setup command for the user to run manually in a Royal TSX connection whose server identity they have verified. Copying the command and opening Royal TSX are allowed; selecting the connection and entering the command remain manual.
