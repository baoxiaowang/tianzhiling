...
轨迹进入离线评测、动态用户、裁判分歧和运行命令见 [聊天轨迹评测](./chat-trajectory-evaluation.md)。高频多轮场景的阶段、动作和退出条件见 [聊天状态协议](./chat-state-protocols.md)。在线 Trace 用于定位运行过程，评测 Case 用于保存可复跑的不可变上下文，两者不要混成同一数据对象。
...
亲人侧主动关心复用人物关心方式、关系阶段、本轮状态和策略动作，不新增模型调用。消息记录保存 `replyCareMotive`、`replyCareFocus` 和 `replyCareStyleSource`，用于区分有关系动机的关心与复述、泛化叮嘱或无目的追问；该计划不进入守卫，也不强制提问。
