...
## Initialization regression repair candidate

The prior a40b68b release was rolled back after contacts returned HTTP 500. A real Midway container reproduced assignment to a getter-only @Config property during MemoryDecisionModelService initialization, even with the feature off. This candidate removes inheritance from OpenAIService: read-only providerConfig and privately owned model configuration/client are separate, without fallback to the character model. Independent model-call attribution is preserved. The maintenance CLI uses the same new configuration input.

The image build now executes check-memory-runtime-init.js against compiled code before any runtime replacement. It checks real Midway injection and lifecycle, contacts controller output and conversation dependency construction in off/active/shadow modes. External providers and storage remain synthetic adapters; this is not a full authenticated HTTP or real chat E2E claim. Production contacts and chat request outcomes must be checked separately after release.

Repair verification: 11 focused suites / 104 tests passed; bounded Node build and offline compiled-artifact gate passed; isolated Mongo persistence/projection/idempotence test passed. Broader conversation/profile suites have 18 failures also reproduced on the exact rollback baseline (10 conversation, 8 profile); do not count those as passes. Production activation and backfill remain independently scoped.
